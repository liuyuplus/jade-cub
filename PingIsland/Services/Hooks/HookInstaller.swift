...
struct HookInstaller {
    ...
    private static func managedMarker(for profile: ManagedHookClientProfile) -> String {
        "Jade Cub managed integration: \(profile.id)"
    }
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        return """
        // \(marker)
        // Generated by Jade Cub. Reinstall from Island settings if you need to refresh it.
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          return {
          name: "Jade Cub",
          description: "Forward OpenCode hook events to Jade Cub.",
          event: async ({ event }) => {
    ...
    static func managedPluginDirectoryFiles(for profile: ManagedHookClientProfile) -> [String: String] {
        ...
        name: ping-island
        version: 1.0.0
        description: Forward Hermes Agent plugin hooks to Jade Cub
        provides_hooks:
          - on_session_start
          - pre_llm_call
          - pre_tool_call
    ...
    static func managedHookDirectoryFiles(
        ...
        ---
        name: \(hookName)
        description: "Forward OpenClaw internal hook events to Jade Cub"
        metadata:
        ...
        const handler = async (event) => {
          ...
          try {
            ...
            // OpenClaw hooks should never fail because Jade Cub is unavailable.
          }
        };
